...
\begin{tikzpicture}[scale=0.7]
    ...
    \addplot[fill=blue!30] coordinates {
        (1,35) (2,12) (3,8) (5,5) (10,3) 
        (20,1.5) (30,0.8) (40,0.5) (50,0.3)
    };
    \end{axis}
\end{tikzpicture}
\end{block}
...
\begin{frame}{t-SNE Troubleshooting Guide}

\begin{tikzpicture}[
    node distance=0.8cm,
    prob/.style={rectangle, draw=red!60, fill=red!5, text width=4cm},
    diag/.style={rectangle, draw=blue!60, fill=blue!5, text width=3cm},
    sol/.style={rectangle, draw=green!60, fill=green!5, text width=3.5cm}
]

% Points explode
\node[prob] (p1) at (0,0) {\textbf{Points explode/diverge?}};
\node[diag, right=of p1] (d1) {Learning rate too high};
\node[sol, right=of d1] (s1) {Reduce $\eta$ by 10×};

% Clusters won't separate
\node[prob, below=1cm of p1] (p2) {\textbf{Clusters won't separate?}};
\node[diag, right=of p2] (d2) {Check iterations\\Check perplexity\\Check learning rate};
\node[sol, right=of d2] (s2) {Run 2000+ iter\\Try perp = 15, 30\\Increase to 200-500};

% Islands
\node[prob, below=1cm of p2] (p3) {\textbf{See "islands" of points?}};
\node[diag, right=of p3] (d3) {Perplexity too low};
\node[sol, right=of d3] (s3) {Minimum perp = 15};

% Blob
\node[prob, below=1cm of p3] (p4) {\textbf{Everything merges?}};
\node[diag, right=of p4] (d4) {Perplexity too high};
\node[sol, right=of d4] (s4) {Maximum perp = n/3};
...
\centering
\begin{tikzpicture}[scale=0.5]
    \draw[fill=blue!30] (-1,1) circle (0.8);
    \draw[fill=red!30] (1,-1) circle (0.8);
\end{tikzpicture}
...
\centering
\begin{tikzpicture}[scale=0.5]
    \draw[fill=blue!30] (-1.5,0) circle (1.2);
    \draw[fill=red!30] (1.5,0) circle (0.5);
\end{tikzpicture}
...
\centering
\begin{tikzpicture}[scale=0.5]
    \draw[fill=blue!30] (-2,0) circle (0.5);
    \draw[fill=green!30] (0,0) circle (0.5);
    \draw[fill=red!30] (2,0) circle (0.5);
    \node at (-2,-1) {A};
    \node at (0,-1) {B};
    \node at (2,-1) {C};
\end{tikzpicture}
...
\begin{tikzpicture}[scale=0.5]
    % Fragmented clusters
    \foreach \x in {0.5,1.2,0.8,1.5} {
        \foreach \y in {0.5,1.0,1.5} {
            \draw[fill=blue!30] (\x,\y) circle (0.15);
        }
    }
    \foreach \x in {2.5,3.0,2.8,3.2} {
        \foreach \y in {0.5,1.0,1.5} {
            \draw[fill=red!30] (\x,\y) circle (0.15);
        }
    }
\end{tikzpicture}
...
\begin{tikzpicture}[scale=0.5]
    ...
    \draw[fill=blue!30] (0.5,1.5) circle (0.6);
    \draw[fill=red!30] (2.5,1.5) circle (0.6);
    \draw[fill=green!30] (1.5,0) circle (0.6);
\end{tikzpicture}
...
\begin{tikzpicture}[scale=0.5]
    % Merged blob
    \draw[fill=gray!30] (1.5,0.8) circle (1.2);
\end{tikzpicture}
